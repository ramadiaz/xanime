...
const RandomAnime = () => {
  ...
  const fetchData = async () => {
    try {
      const randomAPI = await getAnimeResponse("random/anime");
      setRandom(randomAPI);
      ...
      {isLoading ? (
        ...
        <div className="p-6 flex flex-col gap-6 h-full">
          <div className="flex flex-row gap-6">
            <Link href={`/anime/${random.data?.mal_id}`} className="w-max">
              <Image
              ...
              <div className="space-x-2 space-y-2 pb-12 pt-4 md:py-6 max-w-full">
                {random.data?.genres &&
                  random.data?.genres.slice(0, 3).map((genre) => (
                    <button
                      key={genre.mal_id}
                      className="p-2 bg-zinc-300 text-zinc-950 text-sm md:text-md font-semibold pointer-events-none"
                    >
                      {genre.name}
                    </button>
                  ))}
